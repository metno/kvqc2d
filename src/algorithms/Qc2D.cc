...
void Qc2D::calculate_intp_wet_dry(unsigned int index)
{
...
void Qc2D::calculate_intp_h(unsigned int index)
{
    const double RADIUS=6371.0;
    float temp_distance;
    float data_point;
...
/// Algorithm to construct a model value by inverse distance weighting from neighbouring stations.
/// This optionincludes experimental investigation of various uniformity tests.
void Qc2D::intp_temp(unsigned int index)
{
    const double RADIUS=6371.0;
    float temp_distance;
    float data_point;
...
void Qc2D::intp_delaunay(unsigned int index)
{
...
int Qc2D::write_cdf(const std::list<kvalobs::kvStation> & slist)
{
    int result=0;
